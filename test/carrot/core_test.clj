...
(deftest test-subscribe-with-carrot
  (with-open [conn (lhc/connect)
              channel (lch/open conn)]
    (let [qname "message-queue"
          ...
          put-in-dead-letter (atom #{});;we define this atom which will be a global variable where we store the fact that the message has been consumed ok
          ...
          dlq-called (fn [tag] (fn [_] (swap! put-in-dead-letter conj tag)))]
